#ifndef __HASHREHASH_H__
#define __HASHREHASH_H__
...
int rehash_Hash ( Hash* hash, int elemento )
{
    int resp = NULO;
    if ( hash != NULL ) {
        resp = ( ( elemento + 1 ) % hash->tamanho );
    } // end if
    return ( resp );
} // end rehash_Hash ( )
...
bool inserir_Hash ( Hash* hash , int elemento )
{
    ...
    if ( hash != NULL )
    {
        ...
        if ( hash->tabela[index] == NULO )
        {
            ...
            index = rehash_Hash( hash, elemento );
            if ( hash->tabela[index] == NULO )
            {
                hash->tabela[index] = elemento;
                result = true;
